#!/usr/bin/env python3
"""
Final Azure data processing script - processes actual available Azure resources
"""
...
def main():
    ...
    try:
        ...
        # Get network security groups (has data)
        ...
        # Get public IP addresses (has data)
        ...
        # Get network interfaces (has data)
        ...
        # Get app service domains (has data)
        app_domains = session.execute(text("""
            SELECT 
                subscription_id,
                id,
                name,
                ...
                properties,
                tags
            FROM azure_appservice_top_level_domains
            LIMIT 20
        """)).fetchall()
        ...
        print(f"Found {len(app_domains)} app service domains")
        ...
        for resource_list in [storage_accounts, nsgs, vnets, public_ips, nics, app_domains]:
            if resource_list:
        ...
        print("Processing app service domains...")
        domain_count = 0
        for domain in app_domains:
            properties = domain[4] or {}
            resource = CloudResource(
                provider_id=provider.id,
                account_id=account.id,
                resource_id=domain[1] or '',
                resource_name=domain[2] or '',
                resource_type='app_service_domain',
                service_name='appservice',
                region=domain[3] or 'unknown',
                state='active',
                configuration={'properties': properties},
                tags=domain[5] or {},
                public_access=True,  # Domains are typically public
                encryption_enabled=True,
                discovered_at=datetime.utcnow(),
                last_scanned=datetime.utcnow()
            )
            session.add(resource)
            domain_count += 1
        print(f"Created {domain_count} app service domain resources")
        ...
        print(f"  - App Service Domains: {domain_count}")
